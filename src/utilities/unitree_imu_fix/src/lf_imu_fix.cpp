...
// Source: /lf/lowstate (~40 Hz low-frequency channel).
// Unitree quaternion order: [w, x, y, z] → mapped to ROS orientation fields.

class LfImuFix : public rclcpp::Node
{
public:
  LfImuFix() : Node("lf_imu_fix")
  {
    this->declare_parameter<std::string>("input_topic",  "/lf/lowstate");
    this->declare_parameter<std::string>("output_topic", "/lf/imu");
    ...
    RCLCPP_INFO(this->get_logger(), "lf_imu_fix: %s -> %s", in.c_str(), out.c_str());
  }
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<LfImuFix>());
  rclcpp::shutdown();
  return 0;
}
